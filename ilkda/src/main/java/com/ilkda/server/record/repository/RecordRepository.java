...
public interface RecordRepository extends CrudRepository<Record, Long> {
    ...
    @Query("select r from Record r join fetch r.book where r.member = :member and r.complete = :complete")
    List<Record> findAllByMemberAndComplete(@Param("member") Member member, @Param("complete") boolean complete);

    @Query("SELECT COUNT(r) <= :count from Record r WHERE r.member.id = :memberId AND r.complete = :complete")
